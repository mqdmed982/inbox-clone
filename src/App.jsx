...
function App() {
  ...
  // --- 4. واجهة الموقع الأصلية (Row Layout) ---
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-blue-500/30">
      <header className="flex items-center justify-between bg-[#1e293b]/80 backdrop-blur-md p-3 px-6 border-b border-slate-800 sticky top-0 z-50 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="bg-blue-600 p-1.5 rounded-lg shadow-lg shadow-blue-900/40"><Mail size={20} className="text-white" fill="currentColor"/></div>
        ...
        <div className="flex-1 max-w-xl mx-8 relative">
          <input type="text" placeholder="Search subject/sender..." className="w-full bg-[#0f172a] border border-slate-700 text-white py-2 px-10 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500/50 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg text-[10px] font-bold transition shadow-lg uppercase">+ Add Account</button>
          <button onClick={() => {localStorage.removeItem('site_auth'); window.location.reload();}} className="p-2 text-slate-500 hover:text-white transition"><X size={20}/></button>
        </div>
      </header>
      ...
      <main className="p-6 space-y-6 max-w-[1600px] mx-auto">
        {inboxes.map((box) => {
          ...
          return (
            <div key={box.id} className="bg-[#1e293b] border border-slate-800 rounded-xl overflow-hidden shadow-2xl hover:border-slate-700 group transition-all">
              <div className="bg-slate-900/50 p-3 px-5 flex items-center justify-between border-b border-slate-800/50">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => loadRealEmails(box.id)} className="text-[10px] font-bold bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700 uppercase transition hover:bg-slate-700">{loading[box.id] ? "Fetching..." : "Fetch"}</button>
                  <button onClick={() => { if(window.confirm("Delete?")) supabase.from('inboxes').delete().eq('id', box.id).then(() => fetchInboxes()); }} className="text-slate-500 hover:text-red-500 transition p-1"><Trash2 size={18}/></button>
                </div>
              </div>

              <div className="p-4 flex gap-4 overflow-x-auto bg-[#0f172a]/20 custom-scrollbar">
                {filtered.map((mail, i) => (
                  <div key={i} className="min-w-[320px] max-w-[320px] bg-[#1e293b]/50 border border-slate-800 p-3 rounded-lg hover:border-blue-500/30 transition-all shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase border tracking-tighter ${mail.folder === 'SPAM' ? 'bg-red-500/10 text-red-500 border-red-500/30 animate-pulse' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30'}`}>{mail.folder}</span>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-slate-800/50">
                      <div className="bg-slate-900 p-1.5 rounded flex-1 text-[9px] text-slate-400 border border-slate-800 flex items-center justify-between font-mono">
                    ...
                    <div className="flex gap-1.5 mt-2">
                      {['spf', 'dkim', 'dmarc'].map(key => (
                        <div key={key} className={`flex-1 text-center py-1 rounded text-[7px] font-black uppercase border transition-all ${mail[key] === 'PASS' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/30' : mail[key] === 'FAIL' ? 'bg-red-600 text-white border-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)] animate-pulse' : 'bg-slate-800 text-slate-600 border-slate-700'}`}>
                          {key}: {mail[key]}
                        </div>
                      ))}
                    </div>
                ...
                {filtered.length === 0 && <div className="w-full py-8 text-center text-slate-600 text-[10px] italic uppercase tracking-widest">No data synchronized</div>}
              </div>
            </div>
          );
        })}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 flex items-center justify-center p-4 z-[100] backdrop-blur-sm uppercase font-bold text-[10px]">
          <div className="bg-[#1e293b] w-full max-w-md p-8 rounded-2xl border border-slate-700 shadow-2xl relative">
            ...
            <h2 className="text-xl font-black text-center text-blue-400 italic mb-8 uppercase tracking-widest">Add New Inbox</h2>
            <form onSubmit={async (e) => {
              ...
              <select className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none" value={formData.provider} onChange={(e) => setFormData({...formData, provider: e.target.value})}><option value="GMAIL">GMAIL</option><option value="OUTLOOK">OUTLOOK</option></select>
              <input type="text" placeholder="Account Label" className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none" value={formData.user_name} onChange={(e) => setFormData({...formData, user_name: e.target.value})} required />
              <input type="email" placeholder="Email Address" className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required />
              <input type="password" placeholder="App Password" className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl outline-none" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} required />
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-black tracking-widest uppercase mt-4 shadow-lg active:scale-95 transition-all">Save Connection</button>
            </form>
          </div>
        </div>
      )}
